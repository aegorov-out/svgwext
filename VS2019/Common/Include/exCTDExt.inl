...
constexpr HRESULT S_LARGE_FILE		= ERROR_FILE_TOO_LARGE;
constexpr HRESULT S_PATH_TOO_LONG	= ERROR_BUFFER_OVERFLOW;
constexpr HRESULT E_PATH_TOO_LONG	= __HRESULT_FROM_WIN32(ERROR_BUFFER_OVERFLOW);
...
constexpr HRESULT S_UNKNOWN_ERROR	= ERROR_UNIDENTIFIED_ERROR;
constexpr HRESULT E_UNKNOWN_ERROR	= __HRESULT_FROM_WIN32(ERROR_UNIDENTIFIED_ERROR);
#else	// __cplusplus
...
#define S_LARGE_FILE				((HRESULT)ERROR_FILE_TOO_LARGE)
#define S_PATH_TOO_LONG				((HRESULT)ERROR_BUFFER_OVERFLOW)
#define E_PATH_TOO_LONG				((HRESULT)__HRESULT_FROM_WIN32(ERROR_BUFFER_OVERFLOW))
...
#define S_UNKNOWN_ERROR				((HRESULT)ERROR_UNIDENTIFIED_ERROR)
#define E_UNKNOWN_ERROR				((HRESULT)__HRESULT_FROM_WIN32(ERROR_UNIDENTIFIED_ERROR))
#endif	// __cplusplus


#define SUCCEEDEDNOK(hr)			(((HRESULT)(hr)) > 0)
#define FAILEDOROK(hr)				(((HRESULT)(hr)) <= 0)

INLINE _Check_return_ bool_t __fastcall IsOkOrFalse(HRESULT hr)
{
	return (S_OK == hr) | (S_FALSE == hr);
}
#define IsAllocOk(hr)	IsOkOrFalse((HRESULT)(hr))
INLINE _Check_return_ bool_t __fastcall IsAllocSuccess(HRESULT hr)
{
	return IsAllocOk(hr) | (S_EMPTY == hr);
}


typedef enum GENERIC_MATCH_TYPE {
	MATCH_INVALID	= -1,
	MATCH_NONE		= 0,
	MATCH_PARTIAL	= 1,
	MATCH_FULL		= 2
...
typedef const UHALF_PTR*		PCUHALF_PTR;
typedef const HALF_PTR*			PCHALF_PTR;
...
enum WM_APPLICATION_ID CPP_ONLY(: unsigned __int32) {
	WMID_APP_RESERVED_FIRST = WM_APP + 0x0026,	// 0x8026, 32806
	WM_APP_NOOP = WMID_APP_RESERVED_FIRST, WM_APP_PING = WM_APP_NOOP,
	WM_APP_NOTIFY, WM_APP_COMMAND, WM_APP_ERROR, WM_APP_TRAYICON,
	WMID_APP_RESERVED_LAST = WMID_APP_RESERVED_FIRST + 9,
	WMID_APP_USER_FIRST	// 0x8030, 32816
};

#define APP_DATA_ID		0x45	// 69, 'E'
#define APP_COOKIE_ID	APP_DATA_ID
#define APP_TIMER_ID	APP_DATA_ID
#define APP_TIMER0_ID	0x41	// 65, 'A'
#define APP_BKTIMER_ID	APP_TIMER0_ID
